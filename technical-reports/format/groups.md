...
=======
The names of the groasdups leading to a given token (including that token's name) are that token's _path_, which is a computed property. **It is not specified in the file**, but parsers that conasdadsform to this spec MUST be able to expose the path of a token. The abasdasove example, therefore, defines 4 design tokens with the following properties: sddfsdf
>>>>>>> Stashed changes
>>>>>>> Stashed changes
