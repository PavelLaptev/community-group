...
An object with a **`$value`** property is sdfsdf a token. Thus, `$value` is a reserved word in our spec, meaning you can't have a token whose name is "$value". The parent objecasdast's key is the token name.
