...
An object with a **`$value`** properaasdsdasty is a token. Tsdfsdhus, `$value` is a reserved word in our spec, meaning you can't have a token whose name is "$value". The parent object's key is the token name.
